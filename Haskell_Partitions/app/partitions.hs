...
dpPartitions buffer used free _
  | used > free =
      let value = buffer !! free !! free
       in -- in take used buffer ++ [(buffer !! used) ++ [value]] -- ++ drop (used+1) buffer
          insertInLastCell buffer value
  | otherwise =
...
eulerPartitions n
  ...
  | n < 2 = 1
  | otherwise =
      let posArgs = map ((n -) . (`div` 2)) (zipWith (+) [1 .. n] $ map ((3 *) . (^ 2)) [1 .. n])
...
eulerPartitions'' = last . dpEuler []
  where
    ...
    dpEuler buffer n
      ...
      | otherwise =
          let optN = (+) 1 $ (2 * n) `div` 3 -- Cuando k = n, este valor da negativo si n>=1
              posArgs = map ((n -) . (`div` 2)) (zipWith (+) [1 .. optN] $ map ((3 *) . (^ 2)) [1 .. optN])
...
bufferPartitions n = head $ purePartitions [] n
  where
    ...
    inbetweenPartitions buffer used free =
      (buffer !! free)
        - if used < free
  ...
  where
    ...
    dpPentagonal posPentagonal negPentagonal n = value : buffer
      where
        ...
        onlyPos = sum $ zipWith id plusMinus dpPos
  ...
  where
    ...
    dpPentagonal pentagonal n = value
      where
        ...
        plusMinus = cycle [id, id, negate, negate]
        value = sum $ zipWith id plusMinus partitions
  ...
  where
    ...
    vectorPentagonal pentagonal n = value : buffer
      where
        buffer = vectorPentagonal pentagonal (n - 1)
        lessThanN = takeWhile (< n) pentagonal
        dp = map ((V.fromList buffer) V.!) lessThanN -- Horrible opcion
...
pentaUpdateVectorPartitions n = runST $ do
  ...
  MV.read storage n
  where
    -- sqrtN = (+) 1 $ ceiling $ sqrt $ fromIntegral $ div 2*n 3
    ...
    vectorPentagonal storage pentagonal plusMinus n = do
      ...
      dp <- mapM (MV.read storage . (n -)) (takeWhile (<= n) pentagonal)
...
pentaUpdateVectorPartitions''' n = runST $ do
  ...
  where
    ...
    -- vectorPentagonal _ _ _ 0 = return 1
    vectorPentagonal _ [] _ _ = return 1
    vectorPentagonal storage pentagonal@(h : rest) plusMinus n = do
      let optPent = if h > n then rest else pentagonal
      pMinus1 <- vectorPentagonal storage optPent plusMinus (n - 1)
      MV.unsafeWrite storage (n - 1) pMinus1
      dp <- mapM (MV.unsafeRead storage . (n -)) (reverse optPent)
      return $ sum $ zipWith id plusMinus (pMinus1 : dp)

pentaUpdateVectorPartitions'''' :: Int -> Integer -- Substractions and reverses ftw
pentaUpdateVectorPartitions'''' n = runST $ do
  storage <- MV.new n
  vectorPentagonal storage (n : pentagonal) plusMinus
  where
    posPentagonal = 1 : [x + 1 + i + i + i | (i, x) <- zip [1 ..] posPentagonal]
    negPentagonal = zipWith (+) posPentagonal [1 ..]
    pentagonal = reverse $ takeWhile (<= n) $ tail $ foldr (\(x, y) xs -> x : y : xs) [] $ zip posPentagonal negPentagonal
    plusMinus = cycle [id, id, negate, negate]
    ...
    vectorPentagonal _ [0] _ = return 1
    vectorPentagonal _ [1] _ = return 1
    vectorPentagonal storage (n : pentagonal@(h : rest)) plusMinus = do
      let optPent = if h > n then rest else pentagonal
      pMinus1 <- vectorPentagonal storage ((n - 1) : optPent) plusMinus
      MV.unsafeWrite storage (n - 1) pMinus1
      dp <- mapM (MV.unsafeRead storage . (n -)) (reverse optPent)
      return $ sum $ zipWith id plusMinus (pMinus1 : dp)
...
pentaUpdateVectorPartitions''''' n = runST $ do
  storage <- MV.new n
  vectorPentagonal storage optPent n
  where
    posPentagonal = 1 : [x + 1 + i + i + i | (i, x) <- zip [1 ..] posPentagonal]
    negPentagonal = zipWith (+) posPentagonal [1 ..]
    pentagonal = takeWhile (<= n) $ foldr (\(x, y) xs -> x : y : xs) [] $ zip posPentagonal negPentagonal
    plusMinus = take (length pentagonal) $ cycle [id, id, negate, negate]
    optPent = (reverse pentagonal, reverse plusMinus) : [if h > i then (rest, tail pm) else (pent, pm) | (i, (pent@(h : rest), pm)) <- zip [n - 1, n - 2 .. 1] optPent]
    ...
    vectorPentagonal _ _ 0 = return 1
    -- vectorPentagonal _ [] _ _ = return 1

    vectorPentagonal storage ((pent, pm) : rest) i = do
      pMinus1 <- vectorPentagonal storage rest (i - 1)
      MV.write storage (i - 1) pMinus1
      dp <- mapM (MV.read storage . (-) i) pent
      return $ sum $ zipWith id pm dp
...
pentaUpdateVectorPartitions'''''' n = runST $ do
  ...
  where
    ...
    posPentagonal = 1 : [x + 1 + i + i + i | (i, x) <- zip [1 ..] posPentagonal]
    negPentagonal = zipWith (+) posPentagonal [1 ..]
    pentagonal = map (n -) $ takeWhile (<= n) $ foldr (\(x, y) xs -> x : y : xs) [] $ zip posPentagonal negPentagonal
    plusMinus = cycle [id, id, negate, negate]
...
pentaUpdateVectorPartitions''''''' :: Int -> Integer -- The closest to a pure for loop
pentaUpdateVectorPartitions''''''' n = runST $ do
  storage <- MV.new (n + 1)
  mapM_ (vectorPentagonal storage pentagonal plusMinus) [0 .. n]
  MV.read storage n
  where
    -- sqrtN = ceiling $ sqrt $ fromIntegral n
    posPentagonal = 1 : [x + 1 + i + i + i | (i, x) <- zip [1 ..] posPentagonal]
    ...
    -- vectorPentagonal _ _ _ 0 = return 1
    vectorPentagonal storage _ _ 0 = MV.unsafeWrite storage 0 (1 :: Integer)
    vectorPentagonal storage pentagonal plusMinus n = do
      dp <- mapM (MV.unsafeRead storage . (n -)) (takeWhile (<= n) pentagonal)
...
pentaZipperUpdateVectorPartitions n = runST $ do
  ...
  where
    ...
    vectorPentagonal storage t@([], zipper) plusMinus pMinus1 (n : ns) = do
      MV.unsafeWrite storage (n - 1) pMinus1
      dp <- mapM (MV.unsafeRead storage . (n -)) zipper -- Already reversed
    ...
    vectorPentagonal storage t@([p], zipper) plusMinus pMinus1 (n : ns) = do
      MV.unsafeWrite storage (n - 1) pMinus1
      dp <- mapM (MV.unsafeRead storage . (n -)) (reverse zipper) -- Hurts a lot
...
pentaZipperUpdateVectorPartitions' n = runST $ do
  ...
  where
    ...
    vectorPentagonal storage t@(p : ps, zipper) plusMinus pMinus1 (n : ns) = do
      MV.unsafeWrite storage (n - 1) pMinus1
      dp <- mapM (MV.unsafeRead storage . (n -)) zipper
...
pentaDoubleUpdateVectorPartitions n = runST $ do
  ...
  mapM_ (uncurry (MV.write pentVector)) $ zip [0 ..] pentagonal
      ...
      | otherwise = do
          ...
          if nMinus1 < 0
            then return 1
              ...
              mapM_ (MV.modify pentVector (flip (-) 1) . fst) (zip [0 ..] biggerThan0)
...
pentaDoubleUpdateVectorPartitions' n = runST $ do
  ...
  where
    ...
    vectorPentagonal storage pentagonal plusMinus n = do
      ...
      MV.unsafeWrite storage (n - 1) pMinus1
      dp <- mapM (MV.unsafeRead storage) $ takeWhile (>= 0) $ map (n -) pentagonal
      return $ sum $ zipWith (flip (plusMinus V.!)) (pMinus1 : dp) $ cycle [0 .. 3]
      ...
      | otherwise = do
          forI_
            ...
            ( \index -> do
                ...
                MV.unsafeModify storage (addingValue +) index
            )
            used
          dpSamelinePartitions storage (used + 1) n
      where
        -- Adapting from source-code version 0.12.3.0
  ...
  where
    ...
    arrayPentagonal storage pentagonal plusMinus n = sum $ zipWith id plusMinus (pMinus1 : dp)
      where
        ...
        dp = map (storage A.!) $ takeWhile (>= 0) $ map (n -) pentagonal
  ...
  where
    ...
    arrayPentagonal storage pentagonal@(h : rest) plusMinus n = sum $ zipWith id plusMinus (pMinus1 : dp)
      where
        ...
        pMinus1 = storage A.! (n - 1)
        dp = map ((A.!) storage . (n -)) (reverse optPent)
